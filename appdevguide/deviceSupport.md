...
An aiRecord device support that will only be built against 7.0.4 or later
...
First tries to do a an atomic read within the lockset. If this is not possible or necessary (the record does not belong to a lockset or the link does not support locked reads), value and timestamp are read from the input link.  
